...
// route prefix `/catalog/` -> `/` (catalog is the site root here), and
// `hasLogo`/`logoExt` replaced by a bare CAS digest per the wire schema's
// `desc.logo` shape (root.schema.json) — no blob duplication into
// /data/catalog.
...
// Per-card logo extension attempt, keyed by package name — see
// utils/cas.ts's ponytail note on why extension guessing is needed at all.
const logoAttempt = reactive<Record<string, number>>({})

function logoSrc(pkg: PackageSummary): string | null {
  const attempt = logoAttempt[pkg.name] ?? 0
  const ext = LOGO_EXT_CANDIDATES[attempt]
  if (!ext) return null
  return casUrl(pkg.name, pkg.logo, ext)
}

function onLogoError(pkg: PackageSummary) {
  logoAttempt[pkg.name] = (logoAttempt[pkg.name] ?? 0) + 1
}
...
onMounted(async () => {
  try {
    ...
    if (resp.status === 404) {
      // Render pipeline (bot core/render, WP2-F) hasn't populated
      // /data/catalog yet — local dev, or a fresh deploy before the first
      // render run. Friendly empty catalog, never a build/runtime crash.
      data.value = { generated: '', registry: '', packages: [] }
      return
    }
...
<template>
  <div class="catalog">
    ...
    <template v-else-if="data">
      ...
      <div class="grid">
        ...
        >
          <div class="card-header">
            <img
              v-if="logoSrc(pkg)"
              :src="logoSrc(pkg) ?? undefined"
              :alt="`${pkg.title} logo`"
              class="card-logo"
              @error="onLogoError(pkg)"
            >
      ...
      <!-- Empty state — also reached when /data/catalog 404s (see onMounted) -->
      <div v-if="filteredPackages.length === 0" class="empty">
        {{ searchQuery ? 'No packages match your search.' : 'No packages published yet.' }}
      </div>
    </template>
  </div>
</template>
