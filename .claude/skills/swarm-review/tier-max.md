...
- `handover_from_ocx.md` + `design_spec_registry_indirection.md` (§10/§11/§12 authoritative) for every touched surface
...
- **1** `worker-reviewer` (focus: `spec-compliance`, phase: `post-implementation`) — reviews **Implement**-phase output against index anchors (wire contract, workflow security invariants)
