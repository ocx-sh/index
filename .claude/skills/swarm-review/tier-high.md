...
map paths to repo surfaces. Read `product-context.md` +
`handover_from_ocx.md` (+ design spec when contract surfaces
touched). Read language quality rules (`quality-bash.md`,
`quality-python.md`, `quality-security.md`) matching diff
languages.
...
- **1** `worker-reviewer` (focus: `spec-compliance`, phase:
  ...
  output. Anchors: wire-contract shapes additive only
  (`/config.json`, `/p/<ns>/<pkg>.json`), workflow security
  invariants (default-deny permissions, SHA-pinned actions,
  payload validation via env-var indirection), registry truth wins.
...
- `worker-reviewer` (focus: `security`) — **fires when** diff touches
  `.github/workflows/`, auth, secrets, or payload/input handling;
  otherwise skipped
