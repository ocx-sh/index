...
Let `auto` mode pick mixed configs (e.g., "high base with adversarial breadth for medium-size diff touching announce workflow wanting SOTA gap check") without compound-name tiers.
...
Per-tier defaults:
- low → `haiku` (→ `sonnet` when structural markers from `classify.md` "Structural marker signals" fire: `.github/workflows/**`, JSON schemas, `public/config.json` / `p/**` wire-contract surfaces, announce/reconcile auth or payload-validation logic)
- high → `sonnet`
...
| Value | Effect |
|---|---|
| `haiku` | `worker-doc-reviewer` with model=haiku. Triggered when diff touches ≤2 doc files (`README.md`, `public/index.html`). |
...
- high → `off` by default; auto-on when `classify.md` fires `--codex` for One-Way Door signals (breaking change, security-sensitive workflow paths, wire-contract change, published-field/endpoint change)
