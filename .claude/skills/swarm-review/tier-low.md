...
Read diff against resolved baseline (already computed by SKILL.md step 2). Identify single surface touched by matching changed paths against repo layout (`public/`, `p/`, schemas, `.github/workflows/`, `scripts/`). Read `product-context.md` wire-contract section when contract surfaces touched; language quality rule (`quality-bash.md` / `quality-python.md`) otherwise.

**Gate**: Diff fetched, single surface identified, context rule read.
...
- Index pattern compliance (wire-contract compat, workflow security invariants, bash/python quality)
