...
Read plan artifact from `.claude/artifacts/`. Parse classification (Scope, Reversibility, Tier, Overlays), Stub/Specify/Implement/Review phases, Subsystems Touched. Read `product-context.md` + `handover_from_ocx.md` + design spec sections relevant to touched surfaces.

**Gate**: Plan steps parsed; design-record context read.

## Phase 2: Stub

Launch **1** `worker-builder` (focus: `stubbing`, model: sonnet) to create script entrypoints, function shells, schema/fixture skeletons with `raise NotImplementedError` / stub exit. No business logic.

**Gate**: `task verify` passes (JSON well-formed, scripts parse).
...
Stage 1 runs `worker-reviewer` (focus: `spec-compliance`, phase: `post-implementation`) and `worker-reviewer` (focus: `quality`, lens: test-coverage) **in single message with multiple Agent tool calls** so concurrent; if actionable, one builder fix pass + `task verify` before Stage 2. Stage 2 runs `quality`, `security` (if auth/input/crypto/signing touched), `performance` (if hot-path / async touched), `worker-doc-reviewer` (if doc triggers match) **in single message with multiple Agent tool calls** so concurrent.

Rounds 2–3: fresh `worker-builder` fixes actionable findings, `task verify`, re-launch only perspectives with prior actionable findings. Codex code-diff fires when `--codex` resolved on (user flag or classifier-inferred from plan `Reversibility: One-Way Door` / `Overlays: codex=on`); triage per `overlays.md`.
