...
Read plan artifact from `.claude/artifacts/` (or extract scope from free-text target). Parse Stub / Specify / Implement / Review steps. Identify single surface touched; read `product-context.md` wire-contract section when contract surfaces touched.

**Gate**: Plan steps parsed; single surface identified.
...
**Gate**: `task verify` passes. Python-only changes: `uv run ruff check` passes.
...
Stage 1 launches **1** `worker-reviewer` (focus: `spec-compliance`, phase: `post-implementation`); if actionable, one builder fix pass + `task verify`. Stage 2 launches **1** `worker-reviewer` (focus: `quality`); if actionable, one builder fix pass + `task verify`. No Round 2 — `--loop-rounds=1` means one pass.
