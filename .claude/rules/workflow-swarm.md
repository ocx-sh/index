...
1. **Read relevant quality rules FIRST, before any writes.** Path-scoped, auto-load by file type: `.claude/rules/quality-core.md` (universal, always loaded), plus language leaf (`quality-bash.md`, `quality-python.md`) matching files edited. Workflow edits also read `.claude/rules/quality-security.md`. Post-completion self-review no substitute.
2. **Grep existing scripts + workflows for prior art before writing new code.** Check `scripts/` and `.github/` with Grep.
...
- `stubbing`: Public surface only — script entrypoints, signatures, schema/fixture skeletons with `NotImplementedError`/stub exit. Gate: `task verify` passes. Sonnet default.
