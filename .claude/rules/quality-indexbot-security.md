...
bars below fire during planning and before any `bot/**` or `.github/workflows/**`
edit, not just when a file happens to auto-load. Design authority: ADR-6
(`adr_fork_pr_announce.md`, FP-1/FP-4/FP-5/FP-7), ADR-4
(`adr_index_bot_and_workflow_security.md`, BD-3/BD-4/BD-5 + Amendment A1),
`bot/CONTRACTS.md` §12/§14.
...
- Every governance contract G-01..G-20 and every threat class carries a named
  test in `bot/tests/security/` (`test_governance_contracts.py` 1:1 with the
...
golden fixtures (`bot/tests/golden/serializer/`) gate against exactly this.
